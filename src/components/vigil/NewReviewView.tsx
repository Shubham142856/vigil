...
import { Finding, Review, ReviewStatus } from '../../types/vigil';
...
export const NewReviewView: React.FC<NewReviewViewProps> = ({ onCreateReview, onCancel }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 flex flex-col gap-6 text-white select-none">
      ...
      {/* Form or Live Pipeline Execution */}
      {isSimulating ? (
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-white/80">Target Language</label>
              <select
                value={language}
                onChange={(e) => handleLanguageChange(e.target.value as any)}
                className="bg-black border border-white/15 rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-white/40"
              >
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex rounded-lg border border-white/15 bg-black p-1 text-xs">
                <button
                  type="button"
            ...
            {/* Policy Profile */}
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-white/80">Policy Profile</label>
              <select
                value={policyProfile}
          ...
          {inputMethod === 'upload' ? (
            <div className="p-8 rounded-xl border-2 border-dashed border-white/20 bg-white/[0.02] flex flex-col items-center justify-center gap-3 text-center">
              ...
              <label className="mt-2 px-4 py-2 bg-white hover:bg-white/90 text-black rounded-full text-xs font-semibold cursor-pointer transition-colors">
                Select File
                <input
                  type="file"
                  className="hidden"
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                rows={16}
                className="w-full bg-black border border-white/15 rounded-xl p-4 font-mono text-xs text-white/90 leading-relaxed focus:outline-none focus:border-white/40"
          ...
          <div className="p-4 rounded-xl border border-white/10 bg-white/[0.02] flex items-start gap-3">
            <input
              ...
              className="mt-0.5 w-4 h-4 rounded border-white/20 bg-black text-white focus:ring-white/50 cursor-pointer"
            />
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            <button
              type="button"
              onClick={onCancel}
              className="px-5 py-2 rounded-full border border-white/20 bg-transparent text-xs font-medium text-white/70 hover:text-white hover:bg-white/5 transition-colors cursor-pointer"
            >
              Cancel
            </button>
            <button
              ...
              className={`px-5 py-2 rounded-full text-xs font-semibold flex items-center gap-2 transition-colors cursor-pointer ${
                !code.trim() || !consent
