...
import { Feedback } from '../../types/vigil';
...
export const FeedbackButtons: React.FC<FeedbackButtonsProps> = ({
  ...
  return (
    <div className="relative flex flex-col gap-2.5 p-3.5 rounded-xl border border-white/10 bg-white/[0.03]">
      {/* Toast */}
      {showToast && (
        <div className="absolute -top-9 right-2 bg-black border border-emerald-500/40 text-emerald-300 text-xs px-2.5 py-1 rounded-full shadow-lg flex items-center gap-1.5 animate-in fade-in">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className={`p-1.5 rounded-full border text-xs flex items-center gap-1.5 transition-colors cursor-pointer ${
      ...
      {isExpanding && (
        ...
        >
          <div className="flex flex-col gap-1">
            <label className="text-[11px] text-white/60">Classification reason:</label>
            <select
              value={reason}
              onChange={(e) => setReason(e.target.value as Feedback['reason'])}
              className="bg-black border border-white/15 rounded-lg px-2.5 py-1 text-xs text-white focus:outline-none focus:border-white/40"
            >
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              className="bg-black border border-white/15 rounded-lg px-2.5 py-1 text-xs text-white placeholder:text-white/30 focus:outline-none focus:border-white/40"
            />
          </div>
